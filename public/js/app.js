...
function AppModel(JET) {
	...
	this.xRicName = ko.observable("");

	this.xRicYearHigh = ko.observable(0);
	
	this.xRicYearLow = ko.observable(0);

	this.details = ko.computed(function() {
		var xRicName = this.xRicName();
		var xRicYearHigh = this.xRicYearHigh();
		var xRicYearLow = this.xRicYearLow();
		return (xRicName + " Year High: " + xRicYearHigh + " Year Low: " + xRicYearLow);
	}, this);
	...
	this.percentChangeColor = ko.computed(function() {
		var pctChange = this.percentChange();
		return ((pctChange > 0) ? "neg" : (pctChange < 0) ? "pos" : "" );
	}, this);
	...
	function onUpdateHandler(subscription, ric, updatedValues) {
		if (updatedValues["DSPLY_NMLL"]) {
			_self.companyName(updatedValues["DSPLY_NMLL"].formatted);		
		}
		...
		if (updatedValues["X_RIC_NAME"]) {
			_self.xRicName(updatedValues["X_RIC_NAME"].raw);
		}
		if (updatedValues["YRHIGH"]) {
			_self.xRicYearHigh(updatedValues["YRHIGH"].raw);
		}
		if (updatedValues["YRLOW"]) {
			_self.xRicYearLow(updatedValues["YRLOW"].raw);
		}
	}
	...
	this.setRIC = function (ric) {
		...
		_quoteSubscription.rics(ric)
					 .formattedFields("DSPLY_NMLL")
					 .rawFields("CF_LAST", "PCTCHNG", "YRHIGH", "YRLOW", "X_RIC_NAME")
